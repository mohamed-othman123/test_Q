  ...
  private loadDashboard(dashboardId: number): Observable<any> {
    ...
    return this.analyticsService.getAvailableDashboards().pipe(
      ...
      switchMap((dashboards) => {
        ...
        if (this.dashboard) {
          this.dashboardType = this.extractDashboardType(
            this.dashboard.description || this.dashboard.name,
          );
        ...
        return this.analyticsService.getDashboardUrl(
          dashboardId,
          hallIds,
          filters,
        );
